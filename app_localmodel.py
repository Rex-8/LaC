...
def ollama_generate(model: str, prompt: str) -> str:
    try:
        ...
        raise RuntimeError(f"Ollama error: {e.stderr.decode('utf-8')}")
...
def chat():
    ...
    try:
        # STEP 1: Generate SQL with Ollama
        print("\n" + "="*80)
        print("STEP 1: GENERATING SQL (Ollama)")
        print("="*80)
        
        step1_prompt = STEP1_PROMPT.format(user_id=user_id, user_message=user_message)
        sql_query = ollama_generate("gemma3:4b", step1_prompt)
        ...
        # STEP 2: Generate template with Ollama
        print("\n" + "="*80)
        print("STEP 2: GENERATING TEMPLATE (Ollama)")
        print("="*80)
        ...
        llm_response = ollama_generate("gemma3:4b", step2_prompt)
